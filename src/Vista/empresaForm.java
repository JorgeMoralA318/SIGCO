...
public class empresaForm extends javax.swing.JFrame {
    ...
    void actualizar() {
        ...
        dao.Modificar_EmpresaVO(vo);

    }

    //metodo paara cerra formulario actual y abrir principalç
    void cambiar_form() {
         String dat = lblejecucion.getText();
        empresaVista v = new empresaVista();
        v.setResizable(false);
        v.setLocationRelativeTo(null);
        v.lblejecucion.setText(dat);
        v.setTitle("Empresa");
        v.setVisible(true);
        this.dispose();
    }
    ...
    private void obtener_codigo() {
        dao = new EmpresaDAO();
        txtcodigo.setText("EM-" + dao.obtener_id());
        txtcodigo.requestFocus();
    }

    public void listarCodigo(String codigo) {
        ArrayList<EmpresaVO> list = dao.Listar_EmpresaVO(codigo);
        if (list.size() > 0) {
            for (int i = 0; i < list.size(); i++) {
                Object fila[] = new Object[1];
                vo = list.get(i);
                //almacenamos valores en el vector fila
                fila[0] = vo.getEmpresa();
                txtempresa.setText((String) fila[0]);
    ...
    private void txtempresaKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtempresaKeyReleased
        // TODO add your handling code here:
        if (txtempresa.getText().length() >= 4) {
